...
func Lint(moduleDir string, matrix bool) ([]pkg.LinterError, error) {
	...
	// Matrix mode is passed per-case as a source option instead of via the
	// process-global flag, so parallel cases with different settings don't race.
	var opts []static.Option
	if matrix {
		opts = append(opts, static.WithMatrix(true, 0))
	}

	mng := manager.New(cfg, static.NewSource(target, opts...))
	defer mng.Close()
