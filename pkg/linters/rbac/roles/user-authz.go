...
func ObjectUserAuthzClusterRolePath(m *module.Module, object storage.StoreObject) *errors.LintRuleErrorsList {
	...
	if shortPath == UserAuthzClusterRolePath {
		if objectKind != "ClusterRole" {
			return result.WithObjectID(object.Identity()).Error(`Only ClusterRoles can be specified in "templates/user-authz-cluster-roles.yaml"`)
		}
		...
		if !ok {
			return result.WithObjectID(object.Identity()).Error(`User-authz access ClusterRoles should have annotation "user-authz.deckhouse.io/access-level"`)
		}

		expectedName := fmt.Sprintf("d8:user-authz:%s:%s", m.GetName(), strcase.ToKebab(accessLevel))
		if objectName != expectedName {
			return result.WithObjectID(object.Identity()).Errorf("Name of user-authz ClusterRoles should be %q", expectedName)
		}
	}
	return nil
}
