...
*/
func ObjectUserAuthzClusterRolePath(m *module.Module, object storage.StoreObject, lintError *errors.Error) {
	objectKind := object.Unstructured.GetKind()

	shortPath := object.ShortPath()
	if shortPath == UserAuthzClusterRolePath {
		if objectKind != "ClusterRole" {
			lintError.WithObjectID(object.Identity()).Add(`Only ClusterRoles can be specified in "templates/user-authz-cluster-roles.yaml"`)
			return
		}
		...
		if !ok {
			lintError.WithObjectID(object.Identity()).Add(`User-authz access ClusterRoles should have annotation "user-authz.deckhouse.io/access-level"`)
			return
		}

		expectedName := fmt.Sprintf("d8:user-authz:%s:%s", m.GetName(), strcase.ToKebab(accessLevel))
		if objectName != expectedName {
			lintError.WithObjectID(object.Identity()).Add("Name of user-authz ClusterRoles should be %q", expectedName)
			return
		}
	}
}
