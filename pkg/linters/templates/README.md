...
| [listenerset-redirect](#listenerset-redirect) | Requires every ListenerSet host served over HTTPS to also have an HTTP (port 80) listener for the HTTP→HTTPS redirect | ✅ | enabled |
| [httproute-redirect](#httproute-redirect) | Requires every ListenerSet HTTP (port 80) redirect section to be backed by an HTTPRoute that redirects it to HTTPS | ✅ | enabled |
...
### listenerset-redirect

**Purpose:** Ensures that every hostname a `ListenerSet` exposes over HTTPS also has a plain-HTTP (port 80) listener, so a client that arrives over plain HTTP can be redirected to HTTPS instead of being refused. The port 80 listener itself must then be wired to a redirect — see [httproute-redirect](#httproute-redirect).

**What it checks:**

For every `ListenerSet` rendered by the module, each hostname that has at least one `HTTPS` listener must also have an `HTTP` listener on port `80` for the same hostname.

**Why this matters:**

- Without the port 80 listener, plain HTTP requests to an HTTPS-only host get a connection refused / no listener error rather than a redirect to HTTPS.
- It keeps the redirect topology uniform across every public host the module exposes.

❌ **Incorrect** — an HTTPS host with no HTTP (port 80) listener:
...
spec:
  ...
  - name: istio-metadata
    ...
    port: 443
    protocol: HTTPS
    # ❌ no port 80 HTTP listener for metadata.example.com
```

```text
Error: ListenerSet "istio" exposes host "metadata.example.com" over HTTPS but has no HTTP (port 80) listener for the same host
```

✅ **Correct** — every HTTPS host also has a port 80 listener:
...
Exclusions are keyed by **listener section name**, not hostname — hostnames are
rendered from `publicDomainTemplate` and are not known in advance, while section
names are stable literals in the template. A single HTTPS-only section (e.g. a
machine-to-machine metadata endpoint) can be exempted without silencing the rule
for the other sections of the same ListenerSet. `name` (the ListenerSet name) and
`section` are both optional wildcards — omit `name` to match the section in any
ListenerSet.
    ...
    exclude-rules:
      listenerset-redirect:
        - name: istio                # ListenerSet name (optional; omit to match any)
          section: istio-metadata    # HTTPS-only listener section that needs no HTTP redirect listener
```

### httproute-redirect

**Purpose:** Ensures that every plain-HTTP (port 80) redirect section of a `ListenerSet` — one whose hostname is also served over HTTPS — is backed by an `HTTPRoute` that redirects it to HTTPS. It complements [listenerset-redirect](#listenerset-redirect): that rule guarantees the port 80 listener exists, this one guarantees something actually redirects it.

**What it checks:**

For every `ListenerSet` HTTP (port 80) listener whose hostname is also served over HTTPS (a redirect section), there must be an `HTTPRoute` that:

1. has a `spec.parentRefs` entry targeting that `ListenerSet` and its `sectionName`, and
2. has a rule with a `RequestRedirect` filter to HTTPS (`scheme: https`, or `port: 443`).

Hostnames served over plain HTTP only are not redirect sections and need no redirecting HTTPRoute.

❌ **Incorrect** — a redirect section with no HTTPRoute redirecting it:

```yaml
# ListenerSet has an "istio-redirect" port 80 section for an HTTPS host,
# but no HTTPRoute targets sectionName: istio-redirect with a RequestRedirect filter.
```

```text
Error: ListenerSet "istio" section "istio-redirect" is an HTTP (port 80) redirect listener for host "grafana.example.com", but no HTTPRoute redirects it to HTTPS
```

✅ **Correct** — an HTTPRoute redirects the section to HTTPS:
...
spec:
  ...
  - group: gateway.networking.k8s.io
    kind: ListenerSet
    name: istio
    sectionName: istio-redirect  # ✅ the port 80 redirect section
  rules:
  - filters:
...
Exclusions are per-section, so one redirect section can be exempted without
silencing the rule for the other sections of the same ListenerSet. `name` (the
ListenerSet name) and `section` are both optional wildcards.
