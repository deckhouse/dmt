...
func checkImagePullPolicyAlways(md string, object storage.StoreObject, containers []v1.Container) *errors.LintRuleErrorsList {
	...
	if c.ImagePullPolicy != v1.PullAlways {
		return errors.NewLinterRuleList(ID, md).WithObjectID(object.Identity()+"; container = "+c.Name).
			WithWarning(shouldSkipModuleContainer(object.Unstructured.GetName(), c.Name)).
			AddValue(
				c.ImagePullPolicy,
...
func containerNameDuplicates(md string, object storage.StoreObject, containers []v1.Container) *errors.LintRuleErrorsList {
	return checkForDuplicates(
		...
		object,
		containers,
		func(c v1.Container) string { return c.Name },
...
func containerEnvVariablesDuplicates(md string, object storage.StoreObject, containers []v1.Container) *errors.LintRuleErrorsList {
	for i := range containers {
		...
		if err := checkForDuplicates(
			md,
			object,
			c.Env,
			func(e v1.EnvVar) string { return e.Name },
			"Container has two env variables with same name",
			shouldSkipModuleContainer(object.Unstructured.GetName(), c.Name),
		); err != nil {
			return err
		}
	}
...
func checkForDuplicates[T any](md string, object storage.StoreObject, items []T, keyFunc func(T) string, errMsg string, warn bool) *errors.LintRuleErrorsList {
