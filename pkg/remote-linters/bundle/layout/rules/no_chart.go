...
func NewNoChartRule(path string, errorList *errors.LintRuleErrorsList) *NoChartRule {
	return &NoChartRule{
		path:      path,
		errorList: errorList.WithRule(NoChartRuleID),
	}
}
...
func (r *NoChartRule) checkChartFile() {
	...
	r.errorList.WithFilePath(path).Errorf("Chart.yaml found - package root metadata must be defined in package.yaml")
}
