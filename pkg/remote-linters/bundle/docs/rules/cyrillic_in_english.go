...
func NewCyrillicInEnglishRule(path string, errorList *errors.LintRuleErrorsList) *CyrillicInEnglishRule {
	return &CyrillicInEnglishRule{
		path:      path,
		errorList: errorList.WithRule(CyrillicInEnglishRuleID),
	}
}
...
func (r *CyrillicInEnglishRule) checkFile(filePath string) {
	...
	if hasCyr {
		r.errorList.WithFilePath(relPath).WithValue(cyrMsg).Error("English documentation contains cyrillic characters")
	}
}
