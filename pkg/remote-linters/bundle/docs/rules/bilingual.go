...
func NewBilingualRule(path string, errorList *errors.LintRuleErrorsList) *BilingualRule {
	return &BilingualRule{
		path:      path,
		errorList: errorList.WithRule(BilingualRuleID),
	}
}
...
func (r *BilingualRule) Check(_ context.Context) {
	...
	for rel := range fileSet {
		...
		if _, ok := fileSet[ruRel]; !ok {
			r.errorList.WithFilePath(rel).Error("Russian counterpart is missing: need to create a matching .ru.md in docs/")
		}
	}
}
