...
// Package rendercmd implements the "render" command. It discovers every module
...
func renderEdition(modulePath string, globalSchema *spec.Schema, valuesFile, targetDir string) error {
	files, err := modules.RenderModuleForValuesFile(modulePath, globalSchema, valuesFile)
	if err != nil {
		return err
	}
...
func moduleName(modulePath string) string {
	moduleYaml, err := modules.ParseModuleConfigFile(modulePath)
	if err != nil {
		moduleYaml = nil
	}

	chartYaml, err := modules.ParseChartFile(modulePath)
	if err != nil {
		chartYaml = nil
	}

	if name := modules.GetModuleName(moduleYaml, chartYaml); name != "" {
		return name
	}
